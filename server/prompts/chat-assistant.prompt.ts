 ...
 */
export function buildChatAssistantPrompt(ragContext: string, userQuery: string, scopeLabel?: string, webFallbackEnabled?: boolean, plantKnowledgeContext?: string): string {
  ...
  // bulunduysa eklenen, RAG'dan TAMAMEN AYRI bir bölüm. Boşsa mevcut
  // prompt yapısı hiç değişmez.
  const plantKnowledgeSection = plantKnowledgeContext
    ? `\n=== PLANT KNOWLEDGE / DOĞRULANMIŞ BİTKİ BİLGİSİ (KAYNAK: Bitki Bilgi Sözlüğü — elle doğrulanmış, RAG'dan bağımsız) ===\n${plantKnowledgeContext}\n`
    : "";
